                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3 text-center mb-3">
                                    <div class="btn-group status-tabs" role="group">
                                        ...
                                        <button class="btn" :class="getTabClass(1)" @click="setStatusFilter(1)">
                                            Active ({{ activeCount }})
                                        </button>

                                        <button class="btn" :class="getTabClass(0)" @click="setStatusFilter(0)">
                                    ...
                                    <form class="">
                                        <div class="input-group">
                                            <input type="text" class="form-control bg-light small"
                                                ...
                                                aria-describedby="basic-addon2" v-model="searchVendor"
                                                @input="handelSearch">
                                ...
                                <table v-show="!isLoading" class="table table-striped" id="dataTable" width="100%"
                                    ...
                                    <tbody>
                                        ...
                                        <tr v-for="(vendor, index) in vendors" :key="vendor.id">
                                            ...
                                            <td>
                                                <img :src="getImage(vendor.image)" width="50" height="50"
                                                    class="rounded-circle" alt="vendor Image" />
                                            </td>
                                            <td>
                                                {{ vendor.name }}
                                            ...
                                            <td>
                                                <span class="badge badge-success"
                                                    v-if="vendor.status == 1">Active</span>
                                                <span class="badge badge-danger" v-else>Inactive</span>
                                            </td>

                                            <td>
                                                <i class="fas fa-eye text-info mx-2" @click="viewVendor(vendor)"></i>
                                                <i class="fas fa-edit text-primary" @click="editPage(vendor)"></i>
                                                <i class="fas fa-trash text-danger mx-2"
                                                    @click="deleteVendor(vendor, index)"></i>
                                            </td>
        ...
        async deleteVendor(vendor, index) {
            let response = await this.$swal({
                ...
                if (result.value) {
                    try {
                        this.isLoading = true;
                        axios.delete(`vendors/${vendor.id}`).then(response => {
                            this.$swal({
                                title: 'Success!',

                                    ...
                                    <form class="">
                                        <div class="input-group">
                                            <input type="text" class="form-control bg-light small"
                                                ...
                                                aria-describedby="basic-addon2" v-model="searchSize"
                                                @input="handleSizesSearch">
                                ...
                                <table v-show="!isLoading" class="table table-striped" id="dataTable" width="100%"
                                    ...
                                    <tbody>
                                        ...
                                        <tr v-for="(size, index) in sizes" :key="size.id">
                                            ...
                                            <td>
                                                <span v-if="size.image">

                                                    <img :src="getImage(size.image)" width="50" height="50"
                                                        class="rounded-circle" alt="size Image" />
                                                </span>
                                            ...
                                            <td>
                                                {{ size.name }}
                                                <small class="d-block text-muted">{{ size.name }}</small>
                                            </td>
                                            <td>
                                                <span class="badge badge-success"
                                                    v-if="size.status == 'active'">Active</span>
                                                <span class="badge badge-danger" v-else>Inactive</span>
                                            </td>
                                            <!-- ACTION -->
                                            <td>
                                                <i class="fas fa-eye text-info mx-2" @click="viewSize(size)"></i>
                                                <i class="fas fa-edit text-primary" @click="editPage(size)"></i>
                                                <i class="fas fa-trash text-danger mx-2"
                                                    @click="deleteSize(size, index)"></i>
                                            </td>
...
export default {
    ...
    data() {
        return {
            ...
            statusFilter: '', // status filter for active/inactive sizes
    ...
    computed: {
        ...
        activeCount() {
            return this.sizes.filter(c => c.status === "active").length;
        },
        inactiveCount() {
            return this.sizes.filter(c => c.status === "inactive").length;
        }
    },
    methods: {
        getImage(path) {
        ...
        async deleteSize(size, index) {
            let response = await this.$swal({
                ...
                if (result.value) {
                    try {
                        this.isLoading = true;
                        axios.delete(`sizes/${size.id}`).then(response => {
                            this.$swal({
                                title: 'Success!',
        ...
        goToCreatePage() {
            this.$router.push('/admin/sizes/create');
        },
        editPage(size) {
            this.$router.push(`/admin/sizes/${size.id}/edit`);
        },

        viewSize(size) {
            this.$router.push(`/admin/sizes/${size.id}/details`);
        },

        async handleSizesSearch() {
            this.params.search = this.searchSize;
